 ...
 */
public class FlightStatesDeserializer extends StdDeserializer<FlightStates> {
    public FlightStatesDeserializer() {
    ...
    @Override
    public FlightStates deserialize(JsonParser jp, DeserializationContext dc) throws IOException {
        if (jp.getCurrentToken() != null && jp.getCurrentToken() != JsonToken.START_OBJECT) {
            throw dc.mappingException(FlightStates.class);
        }
        try {
            FlightStates res = new FlightStates();
